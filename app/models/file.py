...
class File(db.Model):
    ...
    post_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey('post.id'), index=True)
    note_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey('note.id'), index=True)
